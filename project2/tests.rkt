...
;;CUSTOM EXPRESSION TEST CASES
 ;;basic tests
 (multiple-of-test-b  "multiple-of(0 0)" #t)
 (multiple-of-test "multiple-of(1 0)" #f)
 (multiple-of-test-2 "multiple-of(0 1)" #t)
 (multiple-of-test-3 "multiple-of(0 2)" #t)
 (multiple-of-test-4 "multiple-of(6 3)" #t)
 (multiple-of-test-5 "multiple-of(5 2)" #f)
 (multiple-of-test-6 "multiple-of(6 4)" #f)
 ;;more complex tests
 (multiple-of-complex-test "multiple-of(op(6,'add',2) op(9,'sub',7))" #t)
 (multiple-of-complex-test-2 "multiple-of(op(4,'mult',13) op(44,'div',11))" #t)
 (multiple-of-complex-test-3 "multiple-of(if zero?(3) then 2 else 1 1)" #t)
 (multiple-of-complex-test-4 "multiple-of(my-cond comp(11,'greater',9) then 6, comp(4,'equal',4) then op(13,'mult',1), else 10 2)" #f)
 ;;all-together
 (multiple-of-complex-test-5 "let x = op(5,'sub',1) in let y = op(1,'add',1) in let z = comp(3,'less', 6) in my-cond z then multiple-of(x 2), multiple-of(x 3) then y, else multiple-of(x y)" #t)
