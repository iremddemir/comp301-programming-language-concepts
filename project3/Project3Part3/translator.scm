      ...
      (cases expression exp
        ...
        (var-exp (var)
          ...
          ;;implement translation of var-exp here
                (var-exp
                 ;;convert string to symbol for var-exp identifier
                  (string->symbol
                   ;;concanate the var's string representation with its corresponsinding number
                   (string-append (symbol->string var)
                                  (number->string (- (apply-senv-number senv var) 1)))))
        ...
        (let-exp (var exp1 body)
          ...
          ;;implement translation of let-exp here
           ...
           (string->symbol
            ...
            ;;translate exp1 in senv
            (translation-of exp1 senv)
            ;;translate body in the extended senv
        ...
        (proc-exp (var body)
          ...
          ;;implement translation of proc-exp here   
                  (proc-exp
                   ...
                   ;;translation of body in the extended senv
                   (translation-of body (extend-senv var senv)))
  ...
  ;;;;and finds the occurences of variable var in the environment senv
   (define apply-senv-number
    (lambda (senv var)
      (cond
        ((null? senv) 1)
        ;;if equal then increase the number of occurences
        ((eqv? var (car senv))
         (+ 1 (apply-senv-number (cdr senv) var)))
        ;;if different then continue to search w/o increasing occurences
        (else
