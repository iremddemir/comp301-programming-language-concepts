...
;
     (stack-test1 "let x = newstack() in begin stack-push(x, 10); stack-push(x, 20); stack-push(x,30); stack-size(x) end" 3)
      ...
      ;;test to check if pop returns the num-val
      ;(stack-test6 "let x = newstack() in begin stack-push(x, 10); stack-pop(x) end" 10)
      ;;test to check stack print 
   
      ;;test to check if empty stack pop -1
      ;(stack-test8 "let x = newstack() in begin stack-pop(x) end" -1)
      ;
;
